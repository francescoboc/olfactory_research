...
class Simulation:
    def __init__(self, 
        ...
        if self.real_time_plot:
            ...
            self.axes = plt.subplot(aspect='equal', adjustable='box', xlim=(self.cloud.horizontal_shift-margin, self.cloud.x_max + self.cloud.horizontal_shift+margin), ylim=(self.cloud.vertical_shift-margin, self.cloud.y_max + self.cloud.vertical_shift+margin))
            ...
            colormap='GnBu'
            # colormap='Wistia' 
            self.odor_image = self.axes.imshow(self.cloud.odor>self.swarm.threshold, extent=(self.cloud.horizontal_shift,self.cloud.x_max+self.cloud.horizontal_shift,self.cloud.vertical_shift,self.cloud.y_max+self.cloud.vertical_shift), cmap=colormap, alpha=0.33, origin='lower')
    ...
    def run(self):
        ...
        while time_step < self.tot_time_steps:
            ...
            if self.real_time_plot:
                ...
                self.axes.scatter(self.swarm.agents[0].coordinates[0], self.swarm.agents[0].coordinates[1], marker='.', c='k', s=1) 
...
class Swarm:
    def __init__(self, 
        ...
        self.trust = trust
        self.trust_informed = trust_informed
        self.trust_uninformed = trust_uninformed
        ...
        #             # sum_vel = sum_vel + neighbor.combined_velocity*(1-neighbor.trust)
    ...
    def update(self, time_step):
        ...
        for agent in self.agents:
            ...
            if agent.sniffed:
                ...
                # SURGE (without changing the private vel)
                if self.wind_noise != 0: self.extract_random_wind_estimate()

                # update value of private velocity to move upwind
                vel_surge = -agent.wind_estimate*agent.speed

                # reset counters
                agent.move_diagonal: bool = True
                agent.diagonal_clock: int = 1
                agent.crosswind_clock: int = 1
                agent.crosswind_multi: int = 0

                # update agent's coordinates
                agent.coordinates += agent.speed*agent.dt*normalised(vel_surge)

            else:
                # CAST WITH HACK
                agent.cast(time_step)
                self.update_casting_velocity(agent)
    ...
    def update_casting_velocity(self, agent):

        # sum_vel = agent.combined_velocity
        instant_public_velocity = agent.speed*normalised(agent.combined_velocity.copy())

        # instant_public_velocity = agent.speed*normalised(agent.combined_velocity)
