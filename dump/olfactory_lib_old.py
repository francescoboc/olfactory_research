...
class Simulation:
    def __init__(self, final_time, flow, swarm, cloud, real_time_plot, plot_flow, pause_time, save_frames, elastic, turbulent):
        ...
        # initialise empty list for the trajectory of center of mass
        if save_center_of_mass:
            self.traj_centerofmass = []
        ...
        if self.real_time_plot:
            ...
            # add patches for center of mass and spawn radius
            if self.elastic:
                ...
                self.axes.add_patch(cmass_circle)
            ...
            # vel_arrow = self.swarm.agents[self.nag].combined_velocity
    ...
    def run(self):
        ...
        while time < self.final_time:
            if save_center_of_mass: self.traj_centerofmass.append(self.swarm.center_of_mass.copy())
        ...
        if save_center_of_mass:
            return time, count, success, self.traj_centerofmass
...
class Swarm:
    def __init__(self, n_agents, spawn_center, spawn_radius, decision_time, speed, olfactory_radius, 
        ...
        # list to follow the trajectory of one agent
        self.traj = []
        self.vel_t = []

        # list to save norms of velocities
        self.norms = []
    ...
    def update_random(self, bias):
        for agent in self.agents:
            ...
            agent.private_velocity = bias*np.array([-1,0]) + (1-bias)*v_rand

            # agent.private_velocity /= norm(agent.private_velocity)
            # agent.private_velocity = np.array([np.cos(random_angle), np.sin(random_angle)])
            ...
            agent.combined_velocity = agent.speed*agent.combined_velocity/norm(agent.combined_velocity)
            # update coordinates
            # agent.coordinates += agent.speed*agent.decision_time*agent.combined_velocity
            agent.coordinates += agent.decision_time*agent.combined_velocity
            ...
            if agent.go:
                ...
                if agent.coordinates[0] < self.cloud.source_coordinates[0] + agent.reach_radius:
                    ...
                    if norm(coord_trasl) < agent.visual_radius:
                        agent.combined_velocity = coord_trasl/norm(coord_trasl)*agent.speed

                # calculate combined_velocity (linear comb. of priv. and publ. cues)
                agent.combined_velocity = (1-agent.trust)*agent.private_velocity + agent.trust*agent.public_velocity

                # calculate future coordinates of the agent
                future_coord = agent.coordinates + agent.speed*agent.decision_time*agent.combined_velocity/norm(agent.combined_velocity)
                ...
                agent.combined_velocity = np.array([0, 0])
        ...
        for agent in self.agents:
            if agent.go:
                ...
                agent.acceleration = -1 * ( heavyside*(magnitude - self.spawn_radius) * coord_trasl/magnitude 
                        + agent.inst_velocity - agent.combined_velocity )
    ...
    def update(self):
        ...
        for agent in self.agents:
            ...
            if agent.go:
                ...
                agent.coordinates += agent.speed*agent.decision_time*agent.combined_velocity/norm(agent.combined_velocity)
    ...
    def update_public_velocity(self, agent):
        ...
        if agent.neighbors:
            ...
            # update public velocity
            norm_sum_vel = norm(sum_vel) 
            if norm_sum_vel>0: agent.public_velocity = agent.speed*sum_vel/norm_sum_vel
        # otherwise, set public velocity to 0
        else:
            agent.public_velocity = np.array([0, 0])
    ...
    def compute_order_parameter(self):
        sum_vel = np.zeros(2)
        for agent in self.agents:
            sum_vel += agent.combined_velocity
...
class Agent:
    def __init__(self,label, coordinates, speed, decision_time, olfactory_radius, visual_radius, reach_radius, 
        ...
        self.wind_estimate = np.array([1, 0.0])
    ...
    def cast(self):
        ...
        # if self.clock <= 100/2:
        if self.clock < 1:
            # direction_45 = np.matmul(self._rot_matrix_45, self.wind_estimate)
            direction_45 = -self.wind_estimate
            # direction_45 = np.matmul(self._rot_matrix_neg45, self.wind_estimate)
            self.private_velocity = direction_45*self.speed
            self.clock += 1
        # move crosswind
        else:
            # direction_crosswind = np.matmul(self._rot_matrix_90, self.wind_estimate)
            direction_crosswind = -np.matmul(self._rot_matrix_90, self.wind_estimate)
