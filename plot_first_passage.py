...
try:
    avg_fpt_bins = np.load(f'{hexbin_folder}/avg_fpt_gridsize{gridsize}_offset{offset}.npy', allow_pickle=True))
    print('Average fpt data loaded!')

except:
    print('No average fpt data found!')
    sys.exit()
...
plt.plot(x, y, '+r')
plt.text(bin_coords[bin_idx,0], bin_coords[bin_idx,1]+2, f'{fpt_source:.1f}', ha='center', va='bottom', c='r')
