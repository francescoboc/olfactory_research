...
# plt.title(fr'Highest trust $\beta^*_\rho$ at which $\rho \geq {rate_threshold:.2f}$', pad=10)
plt.text(0.05, 0.95, r'$\beta^*_\rho$', ha='center', va='center', transform=plt.gca().transAxes, fontsize=25)
...
if savefig: plt.gcf().savefig(save_directory + filename + '_rate' + extension, dpi=300)
...
plt.clim(min(trusts)-0.05, max(trusts)+0.05) 
plt.colorbar(hb, label=r'$\beta^*_\tau$, $\beta^*_\rho$', ticks=trusts)
# plt.title(r'Trust $\beta^*_{\tau}$ at which $\tau$ is minimal', pad=10)
plt.text(0.05, 0.95, r'$\beta^*_\tau$', ha='center', va='center', transform=plt.gca().transAxes, fontsize=25)
...
if savefig: plt.gcf().savefig(save_directory + filename + '_fpt' + extension, dpi=300)
