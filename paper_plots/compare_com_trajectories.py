...
for run_n in tqdm(range(n_runs), ascii=' █'):
    ...
    # normv_mean = speed/np.sqrt(1+(1-trust)**2)
    # normv_mean = speed
...
shaded_errorbar(mean_x_sim, mean_y_sim, std_y_sim, label='Simulation', color='b', ls='-', marker='')
...
shaded_errorbar(mean_x_theo, mean_y_theo, std_y_theo, label='Theory', color='k', ls='--', marker='')
